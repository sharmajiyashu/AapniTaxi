...
" class="btn btn-success">Download App</a>
    ...
    public function getCity() {
        ...
        if ($method != 'GET') {
            ...
            //print_r(json_encode($main_final_arr));die;
    ...
    //End Ride API Call
    function addCalculateFareForCurrentCab() {
    try{
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'POST') {
            $output = array(
            ...
            if (isset($riderDetail['rideData']) && count($riderDetail['rideData']) > 0) {
                ...
                //Get Commission data in gst data 
                 ...
                 $promotion_amt = 0;
                 $commission_amt = ($final_price * (double)$getdata['commission'])/100;
                 $gst_amt = ($final_price * (double)$getdata['gst'])/ 100;
                ...
                if(isset($data) && !empty($data)){
                   $data2 = array_values($data);
                    ...
                    foreach($data2 as $key => $val){
                         ...
                         $getdata  = $this->db->get('gst-commission')->row_array();
                         $promotion_amt = 0;
                         $commission_amt = ($val['finalPrice'] * (double)$getdata['commission'])/100;
                         $gst_amt = ($val['finalPrice'] * (double)$getdata['gst'])/ 100;
    ...
    function calculateCabDistance($s_lat, $s_lng, $d_lat, $d_lng){
    ...
    function distanceTimeCalculate($s_lat, $s_lng, $d_lat, $d_lng) {
    ...
    function farCalculateUnderFiveKm($farAmt, $distance, $source_lati, $source_long) {
    ...
    function farCalculateUpperFiveKm($farAmt, $distance, $source_lati, $source_long) {
        foreach ($farAmt as $key => $value) {
            $input_distance = $distance;
            $input_distance = str_replace('m','',$distance);
            $input_distance = str_replace('km','',$distance);
            $input_distance = (double) $input_distance;
            ...
            $aproxTimeMinsHours = $this->distanceTimeCalculate($driver_latitude, $driver_longitude, $source_lati, $source_long);
            $aproxTime = date('Y-m-d H:i:s', strtotime('+' . $aproxTimeMinsHours)); //die;
            $distances = (double) $input_distance - 5;
    ...
    function bookRide() {
        ...
        if ($method != 'POST') {
            ...
            if($driverOnline){
             # Get Driver Near By.
             # If the user has sucked the bike, then the driver will also be the bike.
    ...
    public function searchRide($data){
        $_POST = $data;
        $cabData = $this->RideModel->getCabNearBy($_POST);
        $date = date('Y-m-d');
        if(!empty($cabData)){
    ...
    public function getRiderDetail2() {
        ...
        if ($method != 'POST') {
            ...
            if(!empty($ride_data)){
                ...
                $search_ride_data = $this->searchRide($ride_data_arr);
    ...
    function calculateAllCabDistance($cabData, $postData) {
        # Lokesh Sir Send Code 
        $data = [];
        $data2 = [];
    ...
    function calculateAllCabDistanceUnderTwentyKm($cabData,$desination_latitude,$destination_longitude) {
        $data = [];
        $data2 = [];
    ...
    function saveUserFavoritePalace() {
    ...
    function updateProfile() {
        ...
        if ($method != 'POST') {
            ...
            // if(isset($image_decoded) && !empty($image_decoded)){
            //                 //upload file on server
            //     parent::updateProfilePic($image_decoded,$image_encoded_string,$user_id);
            // }
            if (!empty($validate)) {
                $output = array(
                    'status' => Success,
    ...
    function getUserFavoritePalace() {
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'POST') {
            $output = array(
                'status' => Failure,
    ...
    if($_POST['type'] == 'Driver'){
        $driver_id = $_POST['id'];
        $active_ride_data = $this->db->where('location_status',0)->where('driver_id',$driver_id)->get('cab_ride')->row_array();
    ...
    function getCityNameStateNameByLatitudeLongitude($lat = null, $long = null) {
    ...
    function getDistance($s_lat = null, $s_lng = null, $d_lat = null, $d_lng = null) {
        // $s_lat = '26.822134';
        // $s_lng = '75.774353';
        // $d_lat = '26.917605';
        // $d_lng = '75.816796';
    ...
    function saveDriverBasicDetail() {
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'POST') {
            $output = array(
                'status' => Failure,
    ...
    function uploadDriverFile($postData) {
    ...
    function uploadCabFile($postData) {
    ...
    function uploadUserFile($postData) {
        // $target_dirs = BASEPATH . "../pubilc/driver/";
        // if (!file_exists($target_dirs)) {
        //     mkdir($target_dirs, 0777, true);
        // }
    ...
    public function driverDrivingLicence() {
        ...
        if ($method != 'POST') {
            ...
            if (!empty($update)) {
                $output = array(
                    'status' => Success,
                    'message' => 'driveing Licence detail update successfully',
                    'data' => $update,
                );
            } else {
                $output = array(
    ...
    public function driverAccidentDetail() {
        ...
        if ($method != 'POST') {
            ...
            if (!empty($update)) {
                $output = array(
                    'status' => Success,
                    'message' => 'Accident Detail update successfully',
                    'data' => $update,
                );
            } else {
                $output = array(
    ...
    public function driverPoliceVerification() {
        ...
        if ($method != 'POST') {
            ...
            if (!empty($update)) {
                $output = array(
                    'status' => Success,
                    'message' => 'Police verification detail update successfully',
                    'data' => $update,
                );
            } else {
                $output = array(
    ...
    try{
        ...
        if ($method != 'POST') {
           ...
           if(!empty($_POST)){
              extract($_POST);
                    $cab = $this->db->select('driver_id,price')->where('id',$ride_id)->get('cab_ride')->row_array();
