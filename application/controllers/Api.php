...
class Api extends MY_Controller
{
    public function __construct()
    {
        ...
        $error_log = $now_date.': API_NAME  => '.$url.''.' REQUEST_PARAMS : '.$data;
        if($url == '/Api/saveUserCurrentLocationOnBackground'){
            $stream_clean = $this->security->xss_clean($this->input->raw_input_stream);
            $error_log = $now_date.': API_NAME  => '.$url.''.' REQUEST_PARAMS : '.$stream_clean;
        ...
        try {
            ...
            if ($method != 'POST') {
                ...
                if(!empty($referral_code)){
                    $data = $this->db->where('referral_code',$referral_code)->get('users')->row_array();
                   if(empty($data)){
                        $output = array(
                            'status' => Failure,
    ...
    public function mobileRegister($postData)
    {
        ...
        if ($checkExistDriverTbl > 0) {
            ...
            if (empty($checkExist)) {
                $validate = $this->User_Login_Model->saveUserMobile($postData['mobile'],$otp,$postData);
                if ($validate) {
        ...
        try {
            ...
            if ($method != 'POST') {
                ...
                if ($check_status > 0) {
                    ...
                    $output = array(
                    'status' => Failure,
                    'message' => 'Your account is not active yet. Kindly try again.',
                    'data' => array(),
                );
                }
            }
    ...
    //End Ride API Call
    public function addCalculateFareForCurrentCab_OLD()
    {
        try {
            $method = $this->input->server('REQUEST_METHOD');
            if ($method != 'POST') {
    ...
    public function calculateFareForAllCategories_OLD()
    {
        try {
            $method = $this->input->server('REQUEST_METHOD');
            if ($method != 'POST') {
    ...
    public function bookRide()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($driverOnline) {
                ...
                $user_ride_data = $this->db->where('user_id',$_POST['user_id'])->where_in('location_status', [0,1])->get('cab_ride')->row_array();
                if(empty($user_ride_data)){
                    if (!empty($cabData)) {
                        ...
                        if (!empty($UnderDriverTwentyKm)) {
                            ...
                            $cabAndDriverdata = $this->RideModel->saveRide($_POST, $min_distance, $distance_from_source_destination);
                            $output = array(
                                    'status' => Success,
    ...
    public function getDriverBookings()
    {
        ...
        if ($method != 'POST') {
            ...
            if (!empty($data)) {
                $notification = $this->db->select('ride_id_found_notification')->where('id',$data['otherData']['id'])->get('cab_ride')->row_array();
                $ride_id_found_notification = isset($notification['ride_id_found_notification']) ? $notification['ride_id_found_notification'] : '';

                $output = array(
                    'status' => Success,
                    'message' => 'Ride Details fetch successfully',
                    'data' => $data,
                    'ride_id_found_notification' => $ride_id_found_notification
                );
            } else {
                $output = array(
                    'status' => Failure,
        ...
        if ($method != 'POST') {
            ...
            if ($rideData > 0) {
                ...
                if (!empty($validate)) {
                    ...
                    $notification = $this->db->select('ride_id_found_notification')->where('id',$ride_id)->get('cab_ride')->row_array();
                    $ride_id_found_notification = isset($notification['ride_id_found_notification']) ? $notification['ride_id_found_notification'] : '';
        ...
        if ($method != 'POST') {
            ...
            if (!empty($cabType)) {
                foreach ($cabType as $key => $cab) {
                    $new_array[$key]['title'] = $cab['cab_type_name'];
    ...
    public function saveUserCurrentLocation()
    {
        ...
        //Added by lgarg on 08-sept-2022
        if ($_POST['type'] == 'Driver') {
            $data = $this->db->where('user_id', $_POST['id'])->where('user_type', 'Driver')->get('user_current_locations')->row_array();
            $output['app_update'] = $data['app_update'];
            $output['app_update_url'] = $data['app_update_url'];
            $output['app_update_message'] = $data['app_update_message'];
        }
    ...
    public function saveUserCurrentLocationOnBackground()
    {
        try {
            $method = $this->input->server('REQUEST_METHOD');
            if ($method != 'POST') {
                ...
                // print_r($this->input->raw_input_stream);die;
                // $request_data = json_decode($this->input->raw_input_stream,true);
                $stream_clean = $this->security->xss_clean($this->input->raw_input_stream);
                $request_data = json_decode($stream_clean,true);

                // die("aaa");
                // print_r($request_data);die;
                // $_POST = $request_data;
                $_POST['id'] = $request_data['id'];
                $_POST['type'] = $request_data['type'];
                $_POST['latitude'] = isset($request_data['location']['coords']['latitude']) ? $request_data['location']['coords']['latitude'] : '';
                $_POST['longitude'] = isset($request_data['location']['coords']['longitude']) ? $request_data['location']['coords']['longitude'] : '';
    ...
    public function saveUserCurrentLocationNew()
    {
        try {
            $method = $this->input->server('REQUEST_METHOD');
            if ($method != 'POST') {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    if ($_POST['type'] == 'Driver') {
                        ...
                        $query = $this->db->get('driver');
                        if ($query->num_rows() > 0) {
				$driver_current_location_data = $query->row_array();
				print_r($driver_current_location_data);die;

                            $old_api_count_val = (int) $driver_current_location_data['api_count'];


                            $update = $this->UserModel->updateUserCurrentLocation($_POST,$old_api_count_val);
                            $output = array(
                                'status' => Success,
                                'message' => 'Current Location Updated Successfully',
                                'data' => array(),
    ...
    public function uploadUserProfilePic($postData)
    {
        ...
        foreach ($documents as $key => $val) {
            ...
            if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
                ...
                if(!empty($val["tmp_name"]) || $val["tmp_name"] != ''){
        ...
        try {
            ...
            if ($method != 'POST') {
                ...
                if (!empty($_POST)) {
                    ...
                    if(empty($data)){
                        //check in cab_ride table
                        $data = $this->db->select('payment_type_name as type')->where('id', $ride_id)->get('cab_ride')->row_array();
                    }
                    if (!empty($data)) {
                        $output = array(
                        'status' => Success,
    ...
    */
    ...
    function getCabTypeId($vehicle_type){
        $data = $this->db->where('vehicle_name',$vehicle_type)->get('all_cab_far_price')->row_array();
        $cab_type_id = '';
        if(!empty($data)){
            if(isset($data['cab_type_id'])){
    ...
    /*
    *
    * $data => array(ride_id,destination_latitude,destination_longitude,cab_type_id)
    **/
    function calculateFare($data){
        $next = true;
        if(true){
            if(isset($data['ride_id']) && !empty($data['ride_id']) && $data['ride_id'] != ''){
                $ride_id = $data['ride_id'];

                //verify ride id
                if($this->verifyRideId($ride_id)){
                    //Now fetch ride details
                    $ride_data = $this->fecthRideDetails($ride_id);

                    if(!empty($ride_data)){
                        $vehicle_type = $ride_data['vechicle_name'];
                        $cab_type_id = $this->getCabTypeId($vehicle_type);

                        if (isset($cab_type_id) && !empty($cab_type_id) && $cab_type_id != '') {
                            # User assigned driver changed status online
                            $driver_id = $ride_data['driver_id'];
                            $this->db->where('id', $driver_id)->update('driver', ['driver_current_status' => 'online']);
                            
                            $source_latitude = isset($ride_data['source_latitude']) ? $ride_data['source_latitude'] : '';
                            $source_longitude = isset($ride_data['source_longitude']) ? $ride_data['source_longitude'] : '';

                            if(!empty($source_latitude) && !empty($source_longitude)){
                                
                                if(isset($data['destination_latitude']) && !empty($data['destination_latitude']) && isset($data['destination_longitude']) && !empty($data['destination_longitude'])){
                                    //Now calculate distance
                                    $destination_latitude = isset($data['destination_latitude']) ? $data['destination_latitude'] : '';
                                    $destination_longitude = isset($data['destination_longitude']) ? $data['destination_longitude'] : '';

                                    $ride_distance = $this->calculatRideDistance($source_latitude,$source_longitude,$destination_latitude,$destination_longitude);
                                    
                                    if(!empty($ride_distance) && isset($ride_distance['status'])){
                                        //Now get ride start time and end time
                                        $ride_start_time = $ride_data['ride_start_time'];
                                        $ride_end_time = (isset($ride_data['ride_end_time']) && !empty($ride_data['ride_end_time']) && $ride_data['ride_end_time'] != null) ? $ride_data['ride_end_time'] : date('Y-m-d H:i:s');

                                        //Estimated time of ride while book the cab
                                        $estimated_time_in_min = $ride_data['estimated_time'];
                                        
                                        //Now calculate time difference
                                        $ride_time_diff_in_min = $this->calculateCabEstimateAndActualTimeDiff($ride_start_time,$ride_end_time,$estimated_time_in_min);

                                        //Also get waiting time
                                        $driver_reached_at_location = $ride_data['driver_reached_at_location'];
                                        $driver_reached_location_time = $ride_data['driver_reached_location_time'];
                                        
                                        $ride_waiting_time = $this->calculateWaitingTime($driver_reached_at_location,$driver_reached_location_time,$ride_start_time);
                                
                                        $surcharges = $ride_data['surcharges'];
                                        $promotion_charge = $ride_data['promotion_amt'];
                                        $ride_distance = $ride_distance['distance'];

                                        $final_cab_fare_data = $this->addFare($cab_type_id,$ride_distance,$ride_waiting_time,$surcharges,$promotion_charge,$ride_time_diff_in_min);
                                        
                                        $this->updateCabFare($ride_id,$final_cab_fare_data);

                                        $updateDestination = $this->RideModel->updateDestinationAtTimeOfEndRide($ride_distance,$ride_id,$driver_id,$destination_latitude,$destination_longitude);
                                        
                                        $response_data = $this->returnCabFareResponse($ride_id);
                                        return $response_data;
                                    } else{
                                        return array('status' => false,'message' => 'Unable to calculate ride distance.');
                                        $next = false;
                                    }
                                } else {
                                    return array('status' => false,'message' => 'Destination latitude/longitude can not be blank.');
                                    $next = false;
                                }
                            } else {
                                return array('status' => false,'message' => 'Source latitude/longitude is missing.');
                                $next = false;
                            }
                        } else {
                            return array('status' => false,'message' => 'Source latitude/longitude is missing.');
                        ...
                        return array('status' => false,'message' => 'Ride data should be present.');
                        $next = false;
                    }
                } else {
                    return array('status' => false,'message' => 'Ride id is not valid');
                    $next = false;
                }
            } else {
                return array('status' => false,'message' => 'Ride id can be empty');
                $next = false;
            }
        }
    }
    ...
    function returnCabFareResponse($ride_id){
        $cab_ride_data = $this->RideModel->getCabRideData($ride_id);
        $response_arr = array();
        if (!empty($cab_ride_data)) {
            $response_arr = array(
                'sourceLocation' => array(
                    'latitude' => $cab_ride_data['source_latitude'],
                    'longitude' => $cab_ride_data['source_longitude'],
                ),
                'destinationLocation' => array(
                    'latitude' => $cab_ride_data['destination_latitude'],
                    'longitude' => $cab_ride_data['destination_longitude'],
                ),
                'rideData' => $cab_ride_data
            );
        }
        return $response_arr;
    }

    function updateCabFare($ride_id,$final_cab_fare_data){
        $subtotal_amt = ($final_cab_fare_data['cab_fare_subtotal'] + $final_cab_fare_data['commission_amount'] + $final_cab_fare_data['tax_amount'] + $final_cab_fare_data['waiting_charge'] + $final_cab_fare_data['time_charge']) - $final_cab_fare_data['promotion_amount'];
        $before_tax_amt = ($final_cab_fare_data['cab_fare_subtotal'] + $final_cab_fare_data['commission_amount'] + $final_cab_fare_data['waiting_charge'] + $final_cab_fare_data['time_charge']) - $final_cab_fare_data['promotion_amount'];

        $updateData = array(
            'commission_amt' => $final_cab_fare_data['commission_amount'],
            'gst_amt' => $final_cab_fare_data['tax_amount'],
            'waiting_charge' => $final_cab_fare_data['waiting_charge'],
            'traffic_charge' => $final_cab_fare_data['time_charge'],
            'subtotal_fare_amt' => $subtotal_amt,
            'before_tax_amt' => $before_tax_amt,
            'promotion_amt' => $final_cab_fare_data['promotion_amount'],
            'roundings' => $final_cab_fare_data['roundings'],
            'final_fare_amt' => $final_cab_fare_data['final_cab_fare'],
         );
         $this->db->where('id', $ride_id)->update('cab_ride', $updateData);
         return true;
    }

    function verifyRideId($ride_id){
        return true;
    }

    function fecthRideDetails($ride_id){
       $query = $this->db->where('id',$ride_id)->get('cab_ride');
       if($query->num_rows() > 0){
    ...
    function calculatRideDistance($s_lat,$s_lng,$d_lat,$d_lng){
        $googleMapsApiKey = 'AIzaSyDwn5cCgHITLkJKcCoy_g6hKKcJdjm9IDQ';
        $details_url = "https://maps.googleapis.com/maps/api/directions/json?origin=$s_lat,$s_lng&destination=$d_lat,$d_lng&key=$googleMapsApiKey";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $details_url);
        ...
        return array('distance' => $distance, 'status' => $status);
    }
    
    function convertDistanceInMeter($distance){
    ...
    function calculateCabEstimateAndActualTimeDiff($ride_start_time,$ride_end_time,$estimated_time_in_min){
        //Y-m-d H:i:s -> $ride_start_time
        //Y-m-d H:i:s -> $ride_end_time
        //in minutes  -> $estimated_time
        $estimated_time_in_min = (double) $estimated_time_in_min;
        
        $variable_estimated_end_minutes = '+'.$estimated_time_in_min.' minutes';
        $estimated_ride_end_time_in_ymdhis_format = date("Y-m-d H:i:s", strtotime($variable_estimated_end_minutes, strtotime($ride_start_time)));
        
        $estimated_ride_end_time = strtotime($estimated_ride_end_time_in_ymdhis_format);
        $actual_ride_end_time = strtotime($ride_end_time);
        
        if($actual_ride_end_time > $estimated_ride_end_time){
            $mins = ($actual_ride_end_time - $estimated_ride_end_time) / 60;
        }else{
            $mins = 0;
        }
        return $mins;
    ...
    function addFare($cab_type_id,$ride_distance,$ride_waiting_time,$surcharges,$promotion_charge,$ride_time_diff_in_min){
        //get cab fares
       $cab_fare = $this->getCabFare($cab_type_id);
       $minimum_fare = isset($cab_fare['minimum_fare']) ? $cab_fare['minimum_fare'] : 0;
       $per_km_fare = isset($cab_fare['fare_after_5_km']) ? $cab_fare['fare_after_5_km'] : 0;
       $wating_charge_per_min = isset($cab_fare['waiting_ride_charges']) ? $cab_fare['waiting_ride_charges'] : 0;
       $time_charge_per_min = isset($cab_fare['time_charge']) ? $cab_fare['time_charge'] : 0;
       
       
        //RIDE_WAITING_TIME_BUFFER_IN_MIN
        if($ride_waiting_time <= RIDE_WAITING_TIME_BUFFER_IN_MIN){
            $ride_waiting_time = 0;
        } else {
            $ride_waiting_time = (double) $ride_waiting_time - RIDE_WAITING_TIME_BUFFER_IN_MIN;
        }
        
        //RIDE_TIME_DIFFERENCE_BUFFER_IN_MIN
        if($ride_time_diff_in_min <= RIDE_TIME_DIFFERENCE_BUFFER_IN_MIN){
            $ride_time_diff_in_min = 0;
        } else{
            $ride_time_diff_in_min = (double) $ride_time_diff_in_min - RIDE_TIME_DIFFERENCE_BUFFER_IN_MIN;
        }
        
        //Waiting charge
        $waiting_charge = $ride_waiting_time * $wating_charge_per_min;

        //Time charge OR Traffic Charge
        $traffic_charge = $ride_time_diff_in_min * $time_charge_per_min;

        //Ride distance is in meter so convert it in KM
        $ride_distance = $ride_distance/1000;

        if($surcharges == 0){
            $cab_fare_subtotal = (double) ($minimum_fare + ($ride_distance * $per_km_fare) + $waiting_charge + $traffic_charge - $promotion_charge);
        } else {
            $surcharges_fare_per_km = $per_km_fare + ($per_km_fare * $surcharges)/100;
            $cab_fare_subtotal = (double) ($minimum_fare + ($ride_distance * $surcharges_fare_per_km) + $waiting_charge + $traffic_charge - $promotion_charge);
        }
        ...
        $final_cab_fare = number_format(($cab_fare_subtotal + $cab_fare_commission + $cab_fare_gst),2);

        $final_cab_fare_after_rounding = ceil($final_cab_fare);
        
        $rounding_amount = number_format(($final_cab_fare - $final_cab_fare_after_rounding),2);

        return array(
            'waiting_charge' => number_format($waiting_charge,2),
            'time_charge' => number_format($traffic_charge,2),
            'commission_amount' => number_format($cab_fare_commission,2),
            'tax_amount' => number_format($cab_fare_gst,2),
            'promotion_amount' => $promotion_charge,
            'final_cab_fare' => $final_cab_fare_after_rounding,
            'roundings' => abs($rounding_amount),
            'cab_fare_subtotal' => $cab_fare_subtotal
        );
        // return $final_cab_fare;
    }
    
    function getCabFare($cab_type_id = null){
        $query = $this->db->where('cab_type_id',$cab_type_id)->get('all_cab_far_price');
        if($query->num_rows() > 0){
            return $query->row_array();
        }else{
           return array(); 
    ...
    function calculateWaitingTime($driver_reached_at_location,$driver_reached_location_time,$ride_start_time){
        if($driver_reached_at_location == 1){
            $driver_reached_location_time = strtotime($driver_reached_location_time);
            $ride_start_time = strtotime($ride_start_time);
            $mins = ($ride_start_time - $driver_reached_location_time) / 60;
            if(!empty($mins) || $mins != ''){
               $mins =  abs($mins); 
               return $mins;
            }else if($mins == 0){
    ...
    public function calculateFareForAllCategories(){
        ...
        if($method != 'POST'){
            ...
            $all_cab_fare_data = $this->RideModel->getCabFareOnTheBasisOfCity('Jaipur');
            if(!empty($all_cab_fare_data)){
                foreach($all_cab_fare_data as $key=>$fare_data){
                    $cab_fare = $this->calculateCabFare($fare_data,$distance);
    ...
    function getSurcharges(){
        $surcharges = 0;
        $surcharges_data = $this->db->where('id',1)->get('gst-commission')->row_array();
        if(!empty($surcharges_data)){
            if(isset($surcharges_data['surcharges'])){
                $surcharges = (double) $surcharges_data['surcharges'];
            }
        }
        return $surcharges;
    }
    ...
    public function getDriverProfileDetails(){
        try{
          $method = $this->input->server('REQUEST_METHOD');
          if($method != 'GET'){
              $output = array(
            ...
            if(isset($_GET['driver_id']) && $_GET['driver_id'] != ''){
                $data = $this->db->where('id',$_GET['driver_id'])->get('driver')->row_array();
                if(!empty($data)){
                  $output = array(
                      'status' => Success,
                      'message' => 'Get driver profile details successfully',
                      'data' => $data,
                  );
                }else{
                  $output = array(
      ...
      public function updateRideIdFoundNotification(){
        try{
        $method = $this->input->server('REQUEST_METHOD');
        if($method != 'POST'){
            $output = array(
          ...
          if(isset($_POST['ride_id']) && $_POST['ride_id'] != ''){
              $update_notification = $this->db->where('id',$_POST['ride_id'])->update('cab_ride',['ride_id_found_notification' => 1]);
             if($update_notification > 0){
              $output = array(
                    'status' => Success,
                    'message' => 'Update ride id notification successfully',
                    'data' => array(),
                );   
             }else{
                 $output = array(
                    'status' => Success,
                    'message' => 'Update ride id notification unsuccessfully',
                    'data' => array(),
                ); 
             }
            }else{
    ...
    public function getDriverNotUpdated(){
        $data = $this->db->get('driver')->result_array();
        if(!empty($data)){
            $now_date = Date('Y-m-d H:i:s');
            
            //reduce 15 min in current date time
            $estimated_time = date("Y-m-d H:i:s", strtotime('-15 minutes', strtotime($now_date)));
        
            foreach($data as $key => $val){
                $updated_driver = $this->db->select('updated_at')->where(['user_id' => $val['id'], 'user_type' => 'Driver'])->get('user_current_locations')->row_array();
                if(!empty($updated_driver)){
                    if(strtotime($updated_driver['updated_at']) < strtotime($estimated_time)){
                        $this->db->where('id', $val['id'])->update('driver',['driver_current_status' => 'offline']);
                    }
                }
            }
            $output = array(
