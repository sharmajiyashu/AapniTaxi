        ...
        function getCabNearBy($postData){
            ...
            if(!empty($calculate_distance)){
                 ...
                 if(!empty($user_id)){
                    ...
                    $query = $this->db->query("SELECT ucl.* FROM user_current_locations as ucl INNER JOIN driver on ucl.user_id = driver.id INNER JOIN cab on driver.id = cab.driver_id WHERE ucl.user_type = 'Driver' AND driver.driver_current_status = 'online' AND ucl.user_id IN ($user_ids) AND cab.cab_type_id = $cab_type_id");
